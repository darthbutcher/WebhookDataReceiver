  ...
  switch(sighting.weather){
    ...
    if (verified == MAIN.emotes.yellowQuestion) {
      MAIN.rdmdb.query('SELECT * FROM pokemon WHERE id = ?', [sighting.encounter_id], function (error, record, fields) {
        ...
        pokemon_embed = Embed_Config(pokemon_name,form_name,pokemon_type,sighting.individual_attack,sighting.individual_defense,sighting.individual_stamina,internal_value,sighting.pokemon_level,sighting.cp,gender,height,weight,size,move_name_1,move_type_1,move_name_2,move_type_2,weather_boost,veri,time,mins,secs,sighting.latitude,sighting.longitude,map_url,img_url,pokemon_url,embed_area, possible_cps);
        send_embed();
      });
    } else {
      pokemon_embed = Embed_Config(pokemon_name,form_name,pokemon_type,sighting.individual_attack,sighting.individual_defense,sighting.individual_stamina,internal_value,sighting.pokemon_level,sighting.cp,gender,height,weight,size,move_name_1,move_type_1,move_name_2,move_type_2,weather_boost,verified,hide_time,hide_mins,hide_secs,sighting.latitude,sighting.longitude,map_url,img_url,pokemon_url,embed_area, possible_cps);
      send_embed();
    }
