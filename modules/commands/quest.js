        ...
        if(qIndex<0){ return message.reply('You didn\'t provide a valid reward to subscribe to. Type `'+prefix+'quest help` for available rewards.').then(m => m.delete(10000)).catch(console.error); }
          ...
          MAIN.database.query("SELECT * FROM pokebot.users WHERE user_id = ?", [message.member.id], function (error, user, fields) {
            ...
            MAIN.database.query("UPDATE pokebot.users SET quests = ? WHERE user_id = ?", [userSubs,message.member.id], function (error, user, fields) {
              if(error){ return message.reply('There has been an error, please contact an Admin to fix.').then(m => m.delete(10000)).catch(console.error); }
              else{ return message.reply('Success! You have successfully added `'+MAIN.qConfig.Quest_Rewards[qIndex]+'` to your quest subscriptions.').then(m => m.delete(10000)).catch(console.error); }
            });
          });
        }
      } break;
    case 'remove':
      if(!args[1]){ return message.reply('You didn\'t not specify a reward to remove.').then(m => m.delete(60000)).catch(console.error); }
        ...
        if(qIndex<0){ return message.reply('You didn\'t provide a valid reward to remove. Type `'+prefix+'quest help` for available rewards.').then(m => m.delete(10000)).catch(console.error); }
          ...
          MAIN.database.query("SELECT * FROM pokebot.users WHERE user_id = ?", [message.member.id], function (error, user, fields) {
            if(!user[0].quests){ return message.reply('You are not currently subscribed to '+MAIN.qConfig.Quest_Rewards[qIndex]+'.').then(m => m.delete(10000)).catch(console.error); }
            else{
            ...
            MAIN.database.query("UPDATE pokebot.users SET quests = ? WHERE user_id = ?", [userSubs,message.member.id], function (error, user, fields) {
              if(error){ return message.reply('There has been an error, please contact an Admin to fix.').then(m => m.delete(10000)).catch(console.error); }
              else{ return message.reply('Success! You have successfully removed `'+MAIN.qConfig.Quest_Rewards[qIndex]+'` from your quest subscriptions.').then(m => m.delete(10000)).catch(console.error); }
            });
          });
        }
      } break;
