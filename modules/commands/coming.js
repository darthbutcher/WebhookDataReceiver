...
module.exports.run = async (MAIN, message, raids, count) => {
  ...
  MAIN.pdb.query(`SELECT * FROM lobby_members WHERE gym_id = ?`, [raids.gym_id], function (error, lobby, fields) {
    ...
    if (results === undefined) {
      member_count = 1;
      MAIN.pdb.query(`INSERT INTO lobby_members (gym_id, user_id, count, arrived) VALUES (?,?,?,?) ON DUPLICATE KEY UPDATE count = ?, arrived = ?`, [raids.gym_id, member,member_count,'coming',member_count, 'coming'], function (error, lobby, fields) {
        if(error){ console.error(error); }
      });
      transit_users += member_count;
    ...
    lobby.forEach(function(lobby) {
      ...
      if (lobby.arrived == 'interested' && lobby.user_id == member ) { transit_users += lobby.count; }
      if (lobby.arrived == 'here' && lobby.user_id == member ) {
