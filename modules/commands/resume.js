...
module.exports.run = async (MAIN, message, prefix, discord) => {
  MAIN.pdb.query("UPDATE users SET status = ?, pokemon_status = ?, raids_status = ?, quests_status = ? WHERE user_id = ? AND discord_id = ?", ['ACTIVE', 'ACTIVE', 'ACTIVE', 'ACTIVE', message.member.id, message.guild.id], function (error, user, fields) {
