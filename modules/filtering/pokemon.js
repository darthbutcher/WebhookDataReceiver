...
function CalculatePossibleCPs(MAIN, pokemonID, formID, attack, defense, stamina, level, minCP, maxCP)
{
  ...
  if(!MAIN.pokemon[pokemonID].evolutions){ return possibleCPs; }

  for(var i = 0; i < MAIN.pokemon[pokemonID].evolutions.length; i++) {
    //Check for Evolution Form
    if (formID > 0){
      if(!MAIN.pokemon[pokemonID].forms[formID]){
          evolvedForm = MAIN.pokemon[MAIN.pokemon[pokemonID].evolutions[i]].default_form;
        } else{
          evolvedForm = MAIN.pokemon[pokemonID].forms[formID].evolved_form;
        }
    } else if (MAIN.pokemon[pokemonID].evolved_form){
      evolvedForm = MAIN.pokemon[pokemonID].evolved_form;
    } else { evolvedForm = formID; }

    possibleCPs = possibleCPs.concat(CalculatePossibleCPs(MAIN,MAIN.pokemon[pokemonID].evolutions[i], evolvedForm, attack, defense, stamina, level, minCP, maxCP));
  }
  return possibleCPs;
}
...
function CalculateCP(MAIN, pokemonID, formID, attack , defense, stamina, level)
{
  ...
  if(!MAIN.pokemon[pokemonID].attack)
  {
    if(!MAIN.pokemon[pokemonID].forms[formID] || !MAIN.pokemon[pokemonID].forms[formID].attack){
      console.log("Can't find attack of Pokemon ID: "+pokemonID+' Form:'+formID);
    }
    pokemonAttack = MAIN.pokemon[pokemonID].forms[formID].attack;
  	pokemonDefense = MAIN.pokemon[pokemonID].forms[formID].defense;
  	pokemonStamina = MAIN.pokemon[pokemonID].forms[formID].stamina;
  } else {
    pokemonAttack = MAIN.pokemon[pokemonID].attack;
  	pokemonDefense = MAIN.pokemon[pokemonID].defense;
  	pokemonStamina = MAIN.pokemon[pokemonID].stamina;
  }
...
function CalculatePvPStat(MAIN, pokemonID, formID, level, attack, defense, stamina)
{
    ...
    if(!MAIN.pokemon[pokemonID].attack){
      attack = (attack + MAIN.pokemon[pokemonID].forms[formID].attack) * MAIN.cp_multiplier.CPMultiplier[cpIndex];
      defense = (defense + MAIN.pokemon[pokemonID].forms[formID].defense) * MAIN.cp_multiplier.CPMultiplier[cpIndex];
      stamina = (stamina + MAIN.pokemon[pokemonID].forms[formID].stamina) * MAIN.cp_multiplier.CPMultiplier[cpIndex];

    } else {
      attack = (attack + MAIN.pokemon[pokemonID].attack) * MAIN.cp_multiplier.CPMultiplier[cpIndex];
      defense = (defense + MAIN.pokemon[pokemonID].defense) * MAIN.cp_multiplier.CPMultiplier[cpIndex];
      stamina = (stamina + MAIN.pokemon[pokemonID].stamina) * MAIN.cp_multiplier.CPMultiplier[cpIndex];
    }
